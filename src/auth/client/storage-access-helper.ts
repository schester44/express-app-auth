const storageAccessHelper = `(function() {
  ...
  StorageAccessHelper.prototype.redirectToAppTLD = function(storageAccessStatus) {
    ...
    data = JSON.stringify({
      message: 'AppViewer.API.remoteRedirect',
      data: {
        location: normalizedLink.href,
      }
    });
    window.parent.postMessage(data, this.redirectData.appUrl);
  }
  StorageAccessHelper.prototype.redirectToAppsIndex = function() {
    window.parent.location.href = this.redirectData.appUrl + '/apps';
  }
  ...
  StorageAccessHelper.prototype.grantedStorageAccess = function() {
    try {
      sessionStorage.setItem('app.granted_storage_access', true);
      this.setCookie('app.granted_storage_access=true');
      if (!document.cookie) {
  ...
  StorageAccessHelper.prototype.handleHasStorageAccess = function() {
    if (sessionStorage.getItem('app.granted_storage_access')) {
  ...
  StorageAccessHelper.prototype.handleGetStorageAccess = function() {
    if (sessionStorage.getItem('app.top_level_interaction')) {
  ...
  StorageAccessHelper.prototype.setUpHelper = function() {
    return new ITPHelper({redirectUrl: window.accountOrigin + "/apps/" + window.apiKey + window.returnTo});
  }
  StorageAccessHelper.prototype.setCookieAndRedirect = function() {
    this.setCookie('app.cookies_persist=true');
